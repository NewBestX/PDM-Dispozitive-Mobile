...
import {authConfig, authConfigWithTag, baseUrl, getLogger, withLogs} from '../core';
...
export const getItemsPage: (token: string, page: number, lastEdit: number) => Promise<{items: ItemProps[], lastEdit: number}> = (token, page, lastEdit) => {
  return withLogs(axios.get(itemUrl + "/" + page, authConfigWithTag(token, lastEdit)), 'getItemsPage');
}
